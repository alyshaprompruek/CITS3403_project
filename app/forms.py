...
class AddUnitForm(FlaskForm):
    ...
    # Year drop-down (last 5 years up to current)
    current_year = datetime.now().year
    year_choices = [(str(y), str(y)) for y in range(current_year-5, current_year+1)]
    year = SelectField('Year of Completion', choices=year_choices, default=str(current_year), validators=[DataRequired()])

    # Target score drop-down: 50%,60%,70%,80%,90%,100%
    SCORE_CHOICES = [(str(n), f"{n}%") for n in [50, 60, 70, 80, 90, 100]]
    target_score = SelectField('Target Score (%)', choices=SCORE_CHOICES, validators=[DataRequired()])
